...
import numpy as np
from champion_info import champion_name_from_id, valid_champion_id, get_ids
...
class DraftState:
    ...
    Default draft positions are interpreted as:
        Position 1 -> Top (Tertiary farm)
        Position 2 -> Jungle (Farming support)
        Position 3 -> Middle (Secondary farm)
        Position 4 -> ADC/Marksman (Primary farm)
    ...
    def __init__(self, team, champ_ids = get_ids(), num_positions = 5, draft = Draft('default')):
